...
n = 2
for _ in range(1):
    x = [random.getrandbits(n) for _ in range(4)]
    y = [random.getrandbits(n) for _ in range(4)]
    tweak = [random.getrandbits(n) for _ in range(4)]
    key = [random.getrandbits(n) for _ in range(8)]
    ...
    K = [QuantumRegister(n, name=f'K{i}') for i in range(8)]

    qc = rCircuit(*X, *Y, *K)
    gate = Traxl_enc(X, Y, K, tweak)
    qc.append(gate, list(chain(*gate.inputs)))

    final_circuit = make_circuit(qc, x+y+key, gate.inputs, gate.outputs)
    ...
    subkeys = c_traxl_genkeys(key, n=n)
    true_x, true_y = c_traxl_enc(x, y, subkeys, tweak, n=n)
