...
from pyqrypto.rOperations import rCircuit, rDKRSCarryLookaheadAdder
...
def compare_trax(n=256, trax_nsteps=17):
...
def compare_keyschedule(n=32):
    ...
    s = 0
    for schedule_mode in ['ripple']:# ['lookahead', 'ripple']:
...
def test_trax(n=32):
        ...
        for trax_nsteps in [1,2]:
            for schedule_mode in ['ripple']:#, 'lookahead']:
                for schedule_ancillas in ['same-parallel']:#, 'same-sequential', 'different-parallel', 'different-sequential']: 
                    ...
                    for s in range(trax_nsteps):
                        ...
                        if schedule_mode == 'lookahead':
                            if schedule_ancillas in ['same-parallel', 'same-sequential']:
                                qc.add(key[0], key[1], ancilla_registers[0], mode='lookahead')
                                qc.add(key[0], (RCON[(2*s)%8]%2**n), ancilla_registers[0])
                            elif schedule_ancillas in ['different-parallel', 'different-sequential']:
                                qc.add(key[0], key[1], ancilla_registers[4], mode='lookahead')
                                qc.add(key[0], (RCON[(2*s)%8]%2**n), ancilla_registers[extra_registers_start])
                        elif schedule_mode == 'ripple':
                            qc.add(key[0], key[1], mode='ripple')
                            if schedule_ancillas in ['same-parallel', 'same-sequential']:
                                qc.add(key[0], (RCON[(2*s)%8]%2**n), ancilla_registers[0])
                            if schedule_ancillas in ['different-parallel', 'different-sequential']:
                                qc.add(key[0], (RCON[(2*s)%8]%2**n), ancilla_registers[extra_registers_start])
                        qc.xor(key[2], key[3])
                        qc.xor(key[2], s%2**n)
                        for reg in key:
                        ...
                        #
                        # if schedule_mode == 'lookahead':
                        #     if schedule_ancillas == 'same-parallel':
                        #         qc.add(key[4], key[5], ancilla_registers[1], mode='lookahead')
                        #         qc.add(key[4], (RCON[(2*s+1)%8]%2**n), ancilla_registers[1])
                        #     elif schedule_ancillas == 'same-sequential':
                        #         qc.add(key[4], key[5], ancilla_registers[0], mode='lookahead')
                        #         qc.add(key[4], (RCON[(2*s+1)%8]%2**n), ancilla_registers[0])
                        #     elif schedule_ancillas == 'different-sequential':
                        #         qc.add(key[4], key[5], ancilla_registers[extra_registers_start], mode='lookahead')
                        #         qc.add(key[4], (RCON[(2*s+1)%8]%2**n), ancilla_registers[2])
                        #     elif schedule_ancillas == 'different-parallel':
                        #         qc.add(key[4], key[5], ancilla_registers[extra_registers_start+1], mode='lookahead')
                        #         qc.add(key[4], (RCON[(2*s+1)%8]%2**n), ancilla_registers[3])
                        # elif schedule_mode == 'ripple':
                        #     qc.add(key[4], key[5], mode='ripple')
                        #     if schedule_ancillas == 'same-parallel':
                        #         qc.add(key[4], (RCON[(2*s+1)%8]%2**n), ancilla_registers[1])
                    ...
                    print(qc)
                    print(f"TRAX[{trax_nsteps}]: {alzette_mode}/{schedule_mode}/{schedule_ancillas}:", qc.stats['depth'])

    
#compare_adders(32)
#compare_trax(trax_nsteps=1)
#compare_trax()
#compare_alzette()
#compare_sparkle()
#compare_keyschedule()
