...
class rCircuit(QuantumCircuit):
    ...
    def add(
        ...
        if isinstance(Y, QuantumRegister):
            ...
            if ancillas is None:
                raise CircuitError(
                    "Cannot make a constant adder without ancilla qubits."
                )
...
class rXOR(Gate, rOperation):
    ...
    def __init__(
        self, X: QuantumRegister, Y: QuantumRegister, label: Optional[str] = None
    ) -> None:
        if len(X) != len(Y):
            raise CircuitError(
...
class rConstantDKRSCarryLookaheadAdder(Gate, rOperation):
    ...
    def __init__(
        ...
        if len(ancillas) != self.get_num_ancilla_qubits(self.n):
            raise CircuitError(
                f"Circuit needs {self.get_num_ancilla_qubits(self.n)} ancilla qubits but {len(ancillas)} were given."
            )
        ...
        if self.c != 0:
            ...
            if self.n > 1:
                ...
                uncompute_carry = rDKRSComputeCarry(
...
class rDKRSComputeCarry(Gate, rOperation):
    ...
    def __init__(
        self, P0, G, ancillas: AncillaRegister, label: Optional[str] = None
    ) -> None:
        self.n = len(P0) + 1
        ...
        for t in range(1, floor(log2(self.n))):
            ...
            P.append(
                QuantumRegister(bits=ancillas[ancilla_index : ancilla_index + Pt_size])
            )
...
def make_circuit(
    ...
    for i, input_register in enumerate(input_registers):
        qc.compose(
            rPrepare(input_register, inputs[i]),
            input_register,
            inplace=True,
            front=True,
        )
...
def run_circuit(
    circuit: QuantumCircuit, verbose: bool = False, method="automatic", device="CPU"
) -> Sequence[int]:
